...
class CheckAchStatus extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ach:status';
     ...
     * @var string
     */
    protected $description = 'Check processing Merchant ACH request status';
    ...
    public function handle()
    {
        ...
        if ($today->isWeekend() || in_array($today->toDateString(), $holidays)) {
            echo "\nMerchant ACH can't be checked today since its holiday. \n";
        ...
        if (Auth::loginUsingId($ach_user_id)) {
            ...
            if (Permissions::isAllow('ACH', 'Edit')) {
                ...
                $check_status = PaymentHelper::achCheckStatus();
                Auth::logout();
                if (count($check_status)) {
                    echo "\n".count($check_status)." Merchant ACH status checked successfully \n";
