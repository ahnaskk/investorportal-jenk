...
data() {
   ...
   computed: {
     ...mapState('api', {
      data: s => s.data.investorAssignment,
      loading: (s) => s.loading.investorAssignment,
    ...
    calculatedData(){
      ...
      if(!this.dataColumnLoading && this.dataColumn.data){
        ...
        footer["Participant Amount"] = this.data.participant_amount
        data.push(footer)
      }
      return data
    },
  ...
  created() {
    ...
    if ( !this.dataColumn || !this.dataColumn.data ){
      this.$api({
        url: "/report/investor-assignment-columns",
        field:'dataColumn',
        force: true,
        catchErr: (d) => !d
      })
