...
{{ Breadcrumbs::render('transfer_to_bank',$Investor) }}
<div class="col-md-12">
    <div class="box box-primary">
        {!! Form::open(['url'=>'admin/investors/achRequest/Credit/'.$Investor->id, 'method'=>'POST','id'=>'send_investor_form']) !!}
        <div class="box-body">
            <div class="row">
                <div class="form-group col-md-12">
                    <table with='100%'>
                        <tr>
                            <th>* Send Money to User's Bank Account from Velocity</th>
                        </tr>
                        <tr>
                            <th>* Send Credit Request To Actum</th>
                        </tr>
                        <tr>
                            <th>* For Actum X (Amount) will Add To User's Bank Account </th>
                        </tr>
                        <tr>
                            <th>* For System X (Amount) will Deduct From Velocity</th>
                        </tr>
                    </table>
                </div>
            </div>
            <div class="row">
                ...
                <div class="form-group col-md-3">
                    <select id="transaction_category" name="transaction_category" class="form-control" id="transaction_category" required="">
                    @foreach($transaction_categories as $key => $transaction_category)
                        <option value="{{$key==0?'':$key}}" {{ ($key == 4) ? 'selected' : '' }}>{{$transaction_category}}</option>
                    @endforeach
                    </select>
                    <label for="transaction_category">Transaction Category <span class="validate_star">*</span></label>
                    <span id="invalid-category" />
                </div>
                <div class="form-group col-md-3">
                    {!! Form::number('amount',$amount,['class'=>'form-control','placeholder'=>'Enter the Required Amount','required','id'=> 'amount','min'=>0.01,'max'=>'100000','step'=>'0.01']) !!}
                ...
                <div class="form-group col-md-1">
                    <button type="submit" name="transaction_type" value="credit" class="btn btn-success">Send</button>
                </div>
                <div class="form-group col-md-2">
                    <button type="submit" name="transaction_type" value="same_day_credit" class="btn btn-info">Same Day Send</button>
                </div>
            </div>
            <div class="row">
                <div class="form-group col-md-1">
