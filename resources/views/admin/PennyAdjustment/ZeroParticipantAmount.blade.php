        ...
        <div class="box-body no-padding">
            <div class="row">
                ...
                <div class="col-md-2" style="margin-bottom: -2px;">
                    <div class="input-group">
                        <a href="{{route('PennyAdjustment::RemoveZeroParticipantAmount')}}" class="btn btn-info">Update To Rcode not found</a>
                    </div>
                    <span class="help-block">Update To Rcode not found</span>
                </div>
            </div>
        </div>
        <div class="box-body no-padding">
...
$(document).ready(function(){
    ...
    function format(d) {
        ...
        table+='<tr>';
        table+=      '<th>Date</th>';
        table+=      '<th class="text-right">Payment</th>';
        table+=      '<th class="text-right">Final Participant Share</th>';
        table+='</tr>';
        table+='</thead>';
        table+='<tbody>';
        $.each(d.List,function(key,value){
            ...
            table+='<tr>';
            table+=      '<td>'+value['payment_date']+'</td>';
            table+=      '<td class="text-right">'+value['payment']+'</td>';
            table+=      '<td class="text-right">'+value['final_participant_share']+'</td>';
            table+='</tr>';
        });
